...
- [x] #1 发送消息后，聊天 WebSocket 能成功发出（无因 userId 等导致的发送失败）
- [x] #2 房主发送一条消息后，房主端消息区域显示该条消息
- [x] #3 房主发送一条消息后，成员端消息区域显示该条消息
- [x] #4 成员发送一条消息后，成员端与房主端消息区域均显示该条消息
- [x] #5 消息展示包含发送者标识与内容，多条消息按时间顺序排列
