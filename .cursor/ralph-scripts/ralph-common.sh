...
2. **Step 4 – plan-execute-step (按步执行)** Implement step by step: For each step, invoke the **plan-execute-step** behavior: do one step, verify its acceptance (quick check or run), then move on. **After each step, append a step conclusion to \`.ralph/progress.md\`** (e.g. \`**Step X.Y completed** - <what was done>\` plus bullets: changes made, acceptance result, next step or blocker). This keeps continuity across context rotations. When the overall test command fails, map the failure to a step and fix that step before re-running tests.
...
9. **Update \`.ralph/progress.md\` (Session History)**: (a) **After each implementation step** (plan-execute-step): append a **step conclusion** to Session History: \`**Step X.Y completed** - <one-line what was done>\`, then bullets: files/changes, acceptance result (pass/fail), next step or blocker. (b) **Before signaling complete**: add a **Session N completed** block: \`**Session N completed** - <one-line summary>\`, then a bullet list of what you did. Use the same N as the current iteration. Step conclusions + session block preserve continuity; YOU MUST update progress.md after each step and before complete.
