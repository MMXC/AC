...
def test_task_104():
    ...
    artifact_dir = 'backlog/test-results/task-104'
    os.makedirs(artifact_dir, exist_ok=True)
    scenarios = ['pc1 能成功获取 MediaStream 并通过 addTrack 添加到 PeerConnection', 'pc1 与 pc2 通过本地变量成功交换 offer/answer/ICE，建立 WebRTC 连接', 'pc2 的 <video> 能正常播放从 pc1 发送的远端流', '停止测试时能正确关闭 PeerConnection 和相关 MediaStream 轨道']
    print("=" * 60)
    print(f"测试 TASK-104")
    print("=" * 60)
    print()
    ...
    with sync_playwright() as p:
        ...
        # 测试场景 1: pc1 能成功获取 MediaStream 并通过 addTrack 添加到 PeerConnection
        print(f"\n测试场景 1: pc1 能成功获取 MediaStream 并通过 addTrack 添加到 PeerConnection")
        try:
            # 场景: pc1 能成功获取 MediaStream 并通过 addTrack 添加到 PeerConnection
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 2: pc1 与 pc2 通过本地变量成功交换 offer/answer/ICE，建立 WebRTC 连接
        print(f"\n测试场景 2: pc1 与 pc2 通过本地变量成功交换 offer/answer/ICE，建立 WebRTC 连接")
        try:
            # 场景: pc1 与 pc2 通过本地变量成功交换 offer/answer/ICE，建立 WebRTC 连接
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 3: pc2 的 <video> 能正常播放从 pc1 发送的远端流
        print(f"\n测试场景 3: pc2 的 <video> 能正常播放从 pc1 发送的远端流")
        try:
            # 检查视频元素（通常在点击开始共享后）
            # 如果场景提到"选择内容后"，说明需要先点击开始共享按钮
            if '选择内容后' in 'pc2 的 <video> 能正常播放从 pc1 发送的远端流' or '选择' in 'pc2 的 <video> 能正常播放从 pc1 发送的远端流':
                # 先查找并点击"开始共享"按钮
        ...
        # 测试场景 4: 停止测试时能正确关闭 PeerConnection 和相关 MediaStream 轨道
        print(f"\n测试场景 4: 停止测试时能正确关闭 PeerConnection 和相关 MediaStream 轨道")
        try:
            # 检查停止/关闭功能
            # 查找停止按钮
