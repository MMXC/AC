...
def test_task_120():
    ...
    with sync_playwright() as p:
        ...
        # 测试场景 1: 发送消息后，聊天 WebSocket 能成功发出（无因 userId 等导致的发送失败）
        print(f"\n测试场景 1: 发送消息后，聊天 WebSocket 能成功发出（无因 userId 等导致的发送失败）")
        try:
            # 场景: 发送消息后，聊天 WebSocket 能成功发出（无因 userId 等导致的发送失败）
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 2: 房主发送一条消息后，房主端消息区域显示该条消息
        print(f"\n测试场景 2: 房主发送一条消息后，房主端消息区域显示该条消息")
        try:
            # 场景: 房主发送一条消息后，房主端消息区域显示该条消息
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 3: 房主发送一条消息后，成员端消息区域显示该条消息
        print(f"\n测试场景 3: 房主发送一条消息后，成员端消息区域显示该条消息")
        try:
            # 场景: 房主发送一条消息后，成员端消息区域显示该条消息
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 4: 成员发送一条消息后，成员端与房主端消息区域均显示该条消息
        print(f"\n测试场景 4: 成员发送一条消息后，成员端与房主端消息区域均显示该条消息")
        try:
            # 场景: 成员发送一条消息后，成员端与房主端消息区域均显示该条消息
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 5: 消息展示包含发送者标识与内容，多条消息按时间顺序排列
        print(f"\n测试场景 5: 消息展示包含发送者标识与内容，多条消息按时间顺序排列")
        try:
            # 场景: 消息展示包含发送者标识与内容，多条消息按时间顺序排列
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 6: 自动化测试：双浏览器，任一方发消息，两侧断言消息区域存在对应文本，通过
        print(f"\n测试场景 6: 自动化测试：双浏览器，任一方发消息，两侧断言消息区域存在对应文本，通过")
        try:
            # 场景: 自动化测试：双浏览器，任一方发消息，两侧断言消息区域存在对应文本，通过
            # TODO: 根据具体场景实现测试逻辑
