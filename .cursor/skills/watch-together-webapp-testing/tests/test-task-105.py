...
def test_task_105():
    ...
    5. 点击开始共享后能成功获取 getDisplayMedia 流
    6. 能正确创建 RTCPeerConnection 并将流轨道添加进去
    7. createOffer 成功后能通过 WebSocket 发送格式正确的 WEBRTC_OFFER
    8. WEBRTC_OFFER 包含 toUserId 和 sdp 字段
    """
    # 测试结果截图保存到单独文件夹，便于归档与文档关联
    artifact_dir = 'backlog/test-results/task-105'
    os.makedirs(artifact_dir, exist_ok=True)
    scenarios = ['点击开始共享后能成功获取 getDisplayMedia 流', '能正确创建 RTCPeerConnection 并将流轨道添加进去', 'createOffer 成功后能通过 WebSocket 发送格式正确的 WEBRTC_OFFER', 'WEBRTC_OFFER 包含 toUserId 和 sdp 字段', '点击开始共享后能成功获取 getDisplayMedia 流', '能正确创建 RTCPeerConnection 并将流轨道添加进去', 'createOffer 成功后能通过 WebSocket 发送格式正确的 WEBRTC_OFFER', 'WEBRTC_OFFER 包含 toUserId 和 sdp 字段']
    print("=" * 60)
    print(f"测试 TASK-105")
    print("=" * 60)
    print()
    ...
    with sync_playwright() as p:
        ...
        # 测试场景 1: 点击开始共享后能成功获取 getDisplayMedia 流
        print(f"\n测试场景 1: 点击开始共享后能成功获取 getDisplayMedia 流")
        try:
            # 场景: 点击开始共享后能成功获取 getDisplayMedia 流
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 2: 能正确创建 RTCPeerConnection 并将流轨道添加进去
        print(f"\n测试场景 2: 能正确创建 RTCPeerConnection 并将流轨道添加进去")
        try:
            # 场景: 能正确创建 RTCPeerConnection 并将流轨道添加进去
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 3: createOffer 成功后能通过 WebSocket 发送格式正确的 WEBRTC_OFFER
        print(f"\n测试场景 3: createOffer 成功后能通过 WebSocket 发送格式正确的 WEBRTC_OFFER")
        try:
            # 场景: createOffer 成功后能通过 WebSocket 发送格式正确的 WEBRTC_OFFER
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 4: WEBRTC_OFFER 包含 toUserId 和 sdp 字段
        print(f"\n测试场景 4: WEBRTC_OFFER 包含 toUserId 和 sdp 字段")
        try:
            # 场景: WEBRTC_OFFER 包含 toUserId 和 sdp 字段
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 5: 点击开始共享后能成功获取 getDisplayMedia 流
        print(f"\n测试场景 5: 点击开始共享后能成功获取 getDisplayMedia 流")
        try:
            # 场景: 点击开始共享后能成功获取 getDisplayMedia 流
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 6: 能正确创建 RTCPeerConnection 并将流轨道添加进去
        print(f"\n测试场景 6: 能正确创建 RTCPeerConnection 并将流轨道添加进去")
        try:
            # 场景: 能正确创建 RTCPeerConnection 并将流轨道添加进去
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 7: createOffer 成功后能通过 WebSocket 发送格式正确的 WEBRTC_OFFER
        print(f"\n测试场景 7: createOffer 成功后能通过 WebSocket 发送格式正确的 WEBRTC_OFFER")
        try:
            # 场景: createOffer 成功后能通过 WebSocket 发送格式正确的 WEBRTC_OFFER
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 8: WEBRTC_OFFER 包含 toUserId 和 sdp 字段
        print(f"\n测试场景 8: WEBRTC_OFFER 包含 toUserId 和 sdp 字段")
        try:
            # 场景: WEBRTC_OFFER 包含 toUserId 和 sdp 字段
            # TODO: 根据具体场景实现测试逻辑
