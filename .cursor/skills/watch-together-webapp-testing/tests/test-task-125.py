...
def test_task_125():
    ...
    4. 房主在 #chatInput 输入并点击「发送」后，至少一名成员端 #chatMessages 内出现该条消息文本
    5. agent-browser 或 Playwright 双端测试：房主发送 → 成员端断言 #chatMessages 包含发送内容
    6. 消息展示包含发送者标识（如房主/昵称）与内容
    """
    # 测试结果截图保存到单独文件夹，便于归档与文档关联
    artifact_dir = 'backlog/test-results/task-125'
    os.makedirs(artifact_dir, exist_ok=True)
    scenarios = ['房主在 #chatInput 输入并点击「发送」后，至少一名成员端 #chatMessages 内出现该条消息文本', 'agent-browser 或 Playwright 双端测试：房主发送 → 成员端断言 #chatMessages 包含发送内容', '消息展示包含发送者标识（如房主/昵称）与内容', '房主在 #chatInput 输入并点击「发送」后，至少一名成员端 #chatMessages 内出现该条消息文本', 'agent-browser 或 Playwright 双端测试：房主发送 → 成员端断言 #chatMessages 包含发送内容', '消息展示包含发送者标识（如房主/昵称）与内容']
    print("=" * 60)
    print(f"测试 TASK-125")
    print("=" * 60)
    print()
    ...
    with sync_playwright() as p:
        ...
        # 测试场景 1: 房主在 #chatInput 输入并点击「发送」后，至少一名成员端 #chatMessages 内出现该条消息文本
        print(f"\n测试场景 1: 房主在 #chatInput 输入并点击「发送」后，至少一名成员端 #chatMessages 内出现该条消息文本")
        try:
            # 场景: 房主在 #chatInput 输入并点击「发送」后，至少一名成员端 #chatMessages 内出现该条消息文本
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 2: agent-browser 或 Playwright 双端测试：房主发送 → 成员端断言 #chatMessages 包含发送内容
        print(f"\n测试场景 2: agent-browser 或 Playwright 双端测试：房主发送 → 成员端断言 #chatMessages 包含发送内容")
        try:
            # 场景: agent-browser 或 Playwright 双端测试：房主发送 → 成员端断言 #chatMessages 包含发送内容
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 3: 消息展示包含发送者标识（如房主/昵称）与内容
        print(f"\n测试场景 3: 消息展示包含发送者标识（如房主/昵称）与内容")
        try:
            # 场景: 消息展示包含发送者标识（如房主/昵称）与内容
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 4: 房主在 #chatInput 输入并点击「发送」后，至少一名成员端 #chatMessages 内出现该条消息文本
        print(f"\n测试场景 4: 房主在 #chatInput 输入并点击「发送」后，至少一名成员端 #chatMessages 内出现该条消息文本")
        try:
            # 场景: 房主在 #chatInput 输入并点击「发送」后，至少一名成员端 #chatMessages 内出现该条消息文本
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 5: agent-browser 或 Playwright 双端测试：房主发送 → 成员端断言 #chatMessages 包含发送内容
        print(f"\n测试场景 5: agent-browser 或 Playwright 双端测试：房主发送 → 成员端断言 #chatMessages 包含发送内容")
        try:
            # 场景: agent-browser 或 Playwright 双端测试：房主发送 → 成员端断言 #chatMessages 包含发送内容
            # TODO: 根据具体场景实现测试逻辑
