...
def test_task_108():
    ...
    5. ontrack 回调能正确触发并获取 event.streams[0]
    6. 能调用 VideoPlayer.attachStream(stream) 将远端流传入
    7. 成员端 <video> 能实时播放房主共享画面
    8. 房主停止共享时，成员端能正确 detachStream 并更新 UI
    """
    # 测试结果截图保存到单独文件夹，便于归档与文档关联
    artifact_dir = 'backlog/test-results/task-108'
    os.makedirs(artifact_dir, exist_ok=True)
    scenarios = ['ontrack 回调能正确触发并获取 event.streams[0]', '能调用 VideoPlayer.attachStream(stream) 将远端流传入', '成员端 <video> 能实时播放房主共享画面', '房主停止共享时，成员端能正确 detachStream 并更新 UI', 'ontrack 回调能正确触发并获取 event.streams[0]', '能调用 VideoPlayer.attachStream(stream) 将远端流传入', '成员端 <video> 能实时播放房主共享画面', '房主停止共享时，成员端能正确 detachStream 并更新 UI']
    print("=" * 60)
    print(f"测试 TASK-108")
    print("=" * 60)
    print()
    ...
    with sync_playwright() as p:
        ...
        # 测试场景 1: ontrack 回调能正确触发并获取 event.streams[0]
        print(f"\n测试场景 1: ontrack 回调能正确触发并获取 event.streams[0]")
        try:
            # 场景: ontrack 回调能正确触发并获取 event.streams[0]
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 2: 能调用 VideoPlayer.attachStream(stream) 将远端流传入
        print(f"\n测试场景 2: 能调用 VideoPlayer.attachStream(stream) 将远端流传入")
        try:
            # 检查视频元素（通常在点击开始共享后）
            # 如果场景提到"选择内容后"，说明需要先点击开始共享按钮
            if '选择内容后' in '能调用 VideoPlayer.attachStream(stream) 将远端流传入' or '选择' in '能调用 VideoPlayer.attachStream(stream) 将远端流传入':
                # 先查找并点击"开始共享"按钮
        ...
        # 测试场景 3: 成员端 <video> 能实时播放房主共享画面
        print(f"\n测试场景 3: 成员端 <video> 能实时播放房主共享画面")
        try:
            # 检查视频元素（通常在点击开始共享后）
            # 如果场景提到"选择内容后"，说明需要先点击开始共享按钮
            if '选择内容后' in '成员端 <video> 能实时播放房主共享画面' or '选择' in '成员端 <video> 能实时播放房主共享画面':
                # 先查找并点击"开始共享"按钮
                start_button = page.locator('button:has-text("开始共享"), button:has-text("开始")')
                if start_button.count() > 0 and start_button.first.is_visible():
                    print(f"  ✅ 找到开始共享按钮，点击...")
        ...
        # 测试场景 4: 房主停止共享时，成员端能正确 detachStream 并更新 UI
        print(f"\n测试场景 4: 房主停止共享时，成员端能正确 detachStream 并更新 UI")
        try:
            # 检查停止/关闭功能
            # 查找停止按钮
        ...
        # 测试场景 5: ontrack 回调能正确触发并获取 event.streams[0]
        print(f"\n测试场景 5: ontrack 回调能正确触发并获取 event.streams[0]")
        try:
            # 场景: ontrack 回调能正确触发并获取 event.streams[0]
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 6: 能调用 VideoPlayer.attachStream(stream) 将远端流传入
        print(f"\n测试场景 6: 能调用 VideoPlayer.attachStream(stream) 将远端流传入")
        try:
            # 检查视频元素（通常在点击开始共享后）
            # 如果场景提到"选择内容后"，说明需要先点击开始共享按钮
        ...
        # 测试场景 8: 房主停止共享时，成员端能正确 detachStream 并更新 UI
        print(f"\n测试场景 8: 房主停止共享时，成员端能正确 detachStream 并更新 UI")
        try:
            # 检查停止/关闭功能
            # 查找停止按钮
