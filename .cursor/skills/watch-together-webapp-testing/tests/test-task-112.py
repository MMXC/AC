...
def test_task_112():
    ...
    artifact_dir = 'backlog/test-results/task-112'
    os.makedirs(artifact_dir, exist_ok=True)
    scenarios = ['房主未开始共享时，成员端显示「等待房主开始共享...」', 'WebRTC 建立并收到远端流时，显示视频和「正在播放房主画面」', '房主停止共享或连接断开时，停止播放并显示「房主已停止共享」或错误提示', '状态文案与实际连接状态一致']
    print("=" * 60)
    print(f"测试 TASK-112")
    print("=" * 60)
    print()
    ...
    with sync_playwright() as p:
        ...
        # 测试场景 1: 房主未开始共享时，成员端显示「等待房主开始共享...」
        print(f"\n测试场景 1: 房主未开始共享时，成员端显示「等待房主开始共享...」")
        try:
            # 场景: 房主未开始共享时，成员端显示「等待房主开始共享...」
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 2: WebRTC 建立并收到远端流时，显示视频和「正在播放房主画面」
        print(f"\n测试场景 2: WebRTC 建立并收到远端流时，显示视频和「正在播放房主画面」")
        try:
            # 检查视频元素（通常在点击开始共享后）
            # 如果场景提到"选择内容后"，说明需要先点击开始共享按钮
            if '选择内容后' in 'WebRTC 建立并收到远端流时，显示视频和「正在播放房主画面」' or '选择' in 'WebRTC 建立并收到远端流时，显示视频和「正在播放房主画面」':
                # 先查找并点击"开始共享"按钮
        ...
        # 测试场景 3: 房主停止共享或连接断开时，停止播放并显示「房主已停止共享」或错误提示
        print(f"\n测试场景 3: 房主停止共享或连接断开时，停止播放并显示「房主已停止共享」或错误提示")
        try:
            # 检查停止/关闭功能
            # 查找停止按钮
