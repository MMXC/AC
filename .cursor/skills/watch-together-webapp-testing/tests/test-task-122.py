...
def test_task_122():
    ...
    测试场景:
    1. 房主在 #chatInput 输入并点击「发送」后，至少一名成员端 #chatMessages 内出现该条消息文本（服务端广播已实现；成员端由 chat 收包渲染，sync 收到 CHAT_MESSAGE 时转发给 window.handleWebSocketMessage 以兼容先连 sync 的场景）
    2. agent-browser 或 Playwright 双端测试：房主发送 → 成员端断言 #chatMessages 包含发送内容（test-122.py 已添加，需在本地/CI 通过）
    3. 消息展示包含发送者标识（如房主/昵称）与内容（renderMessage 已展示 nickname 与 content）
    """
    # 测试结果截图保存到单独文件夹，便于归档与文档关联
    artifact_dir = 'backlog/test-results/task-122'
    os.makedirs(artifact_dir, exist_ok=True)
    scenarios = ['房主在 #chatInput 输入并点击「发送」后，至少一名成员端 #chatMessages 内出现该条消息文本（服务端广播已实现；成员端由 chat 收包渲染，sync 收到 CHAT_MESSAGE 时转发给 window.handleWebSocketMessage 以兼容先连 sync 的场景）', 'agent-browser 或 Playwright 双端测试：房主发送 → 成员端断言 #chatMessages 包含发送内容（test-122.py 已添加，需在本地/CI 通过）', '消息展示包含发送者标识（如房主/昵称）与内容（renderMessage 已展示 nickname 与 content）']
    print("=" * 60)
    print(f"测试 TASK-122")
    print("=" * 60)
    print()
    ...
    with sync_playwright() as p:
        ...
        # 测试场景 1: 房主在 #chatInput 输入并点击「发送」后，至少一名成员端 #chatMessages 内出现该条消息文本（服务端广播已实现；成员端由 chat 收包渲染，sync 收到 CHAT_MESSAGE 时转发给 window.handleWebSocketMessage 以兼容先连 sync 的场景）
        print(f"\n测试场景 1: 房主在 #chatInput 输入并点击「发送」后，至少一名成员端 #chatMessages 内出现该条消息文本（服务端广播已实现；成员端由 chat 收包渲染，sync 收到 CHAT_MESSAGE 时转发给 window.handleWebSocketMessage 以兼容先连 sync 的场景）")
        try:
            # 场景: 房主在 #chatInput 输入并点击「发送」后，至少一名成员端 #chatMessages 内出现该条消息文本（服务端广播已实现；成员端由 chat 收包渲染，sync 收到 CHAT_MESSAGE 时转发给 window.handleWebSocketMessage 以兼容先连 sync 的场景）
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 2: agent-browser 或 Playwright 双端测试：房主发送 → 成员端断言 #chatMessages 包含发送内容（test-122.py 已添加，需在本地/CI 通过）
        print(f"\n测试场景 2: agent-browser 或 Playwright 双端测试：房主发送 → 成员端断言 #chatMessages 包含发送内容（test-122.py 已添加，需在本地/CI 通过）")
        try:
            # 场景: agent-browser 或 Playwright 双端测试：房主发送 → 成员端断言 #chatMessages 包含发送内容（test-122.py 已添加，需在本地/CI 通过）
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 3: 消息展示包含发送者标识（如房主/昵称）与内容（renderMessage 已展示 nickname 与 content）
        print(f"\n测试场景 3: 消息展示包含发送者标识（如房主/昵称）与内容（renderMessage 已展示 nickname 与 content）")
        try:
            # 场景: 消息展示包含发送者标识（如房主/昵称）与内容（renderMessage 已展示 nickname 与 content）
            # TODO: 根据具体场景实现测试逻辑
