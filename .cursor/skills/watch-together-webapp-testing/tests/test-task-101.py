...
def test_task_101():
    ...
    5. VideoPlayer 可多次 attachStream / detachStream 而无内存泄漏或挂死
    6. 附加合法 MediaStream 时，成员端 <video> 能正常播放画面和（可选）音频
    7. detachStream 后，视频区域清空或显示「等待流」状态
    8. 组件不依赖 WebRTC 细节，只关心 MediaStream 对象
    """
    # 测试结果截图保存到单独文件夹，便于归档与文档关联
    artifact_dir = 'backlog/test-results/task-101'
    os.makedirs(artifact_dir, exist_ok=True)
    scenarios = ['VideoPlayer 可多次 attachStream / detachStream 而无内存泄漏或挂死', '附加合法 MediaStream 时，成员端 <video> 能正常播放画面和（可选）音频', 'detachStream 后，视频区域清空或显示「等待流」状态', '组件不依赖 WebRTC 细节，只关心 MediaStream 对象', 'VideoPlayer 可多次 attachStream / detachStream 而无内存泄漏或挂死', '附加合法 MediaStream 时，成员端 <video> 能正常播放画面和（可选）音频', 'detachStream 后，视频区域清空或显示「等待流」状态', '组件不依赖 WebRTC 细节，只关心 MediaStream 对象']
    print("=" * 60)
    print(f"测试 TASK-101")
    print("=" * 60)
    print()
    ...
    with sync_playwright() as p:
        ...
        # 测试场景 1: VideoPlayer 可多次 attachStream / detachStream 而无内存泄漏或挂死
        print(f"\n测试场景 1: VideoPlayer 可多次 attachStream / detachStream 而无内存泄漏或挂死")
        try:
            # 检查视频元素（通常在点击开始共享后）
            # 如果场景提到"选择内容后"，说明需要先点击开始共享按钮
            if '选择内容后' in 'VideoPlayer 可多次 attachStream / detachStream 而无内存泄漏或挂死' or '选择' in 'VideoPlayer 可多次 attachStream / detachStream 而无内存泄漏或挂死':
                # 先查找并点击"开始共享"按钮
        ...
        # 测试场景 2: 附加合法 MediaStream 时，成员端 <video> 能正常播放画面和（可选）音频
        print(f"\n测试场景 2: 附加合法 MediaStream 时，成员端 <video> 能正常播放画面和（可选）音频")
        try:
            # 检查视频元素（通常在点击开始共享后）
            # 如果场景提到"选择内容后"，说明需要先点击开始共享按钮
            if '选择内容后' in '附加合法 MediaStream 时，成员端 <video> 能正常播放画面和（可选）音频' or '选择' in '附加合法 MediaStream 时，成员端 <video> 能正常播放画面和（可选）音频':
                # 先查找并点击"开始共享"按钮
                start_button = page.locator('button:has-text("开始共享"), button:has-text("开始")')
                if start_button.count() > 0 and start_button.first.is_visible():
                    print(f"  ✅ 找到开始共享按钮，点击...")
        ...
        # 测试场景 3: detachStream 后，视频区域清空或显示「等待流」状态
        print(f"\n测试场景 3: detachStream 后，视频区域清空或显示「等待流」状态")
        try:
            # 检查视频元素（通常在点击开始共享后）
            # 如果场景提到"选择内容后"，说明需要先点击开始共享按钮
            if '选择内容后' in 'detachStream 后，视频区域清空或显示「等待流」状态' or '选择' in 'detachStream 后，视频区域清空或显示「等待流」状态':
                # 先查找并点击"开始共享"按钮
                start_button = page.locator('button:has-text("开始共享"), button:has-text("开始")')
                if start_button.count() > 0 and start_button.first.is_visible():
                    print(f"  ✅ 找到开始共享按钮，点击...")
        ...
        # 测试场景 4: 组件不依赖 WebRTC 细节，只关心 MediaStream 对象
        print(f"\n测试场景 4: 组件不依赖 WebRTC 细节，只关心 MediaStream 对象")
        try:
            # 场景: 组件不依赖 WebRTC 细节，只关心 MediaStream 对象
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 5: VideoPlayer 可多次 attachStream / detachStream 而无内存泄漏或挂死
        print(f"\n测试场景 5: VideoPlayer 可多次 attachStream / detachStream 而无内存泄漏或挂死")
        try:
            # 检查视频元素（通常在点击开始共享后）
            # 如果场景提到"选择内容后"，说明需要先点击开始共享按钮
            ...
            for selector in video_selectors:
                ...
                if videos:
                    ...
                    if is_playing:
                        print(f"  ✅ 视频正在播放")
                        test_results.append(("场景 5", True, "视频正在播放"))
                    else:
                        print(f"  ⚠️  视频未播放（可能用户还未选择屏幕源）")
                        test_results.append(("场景 5", None, "视频未播放（需要用户选择屏幕源）"))
                    video_found = True
                    break
            
            if not video_found:
                print(f"  ❌ 未找到视频元素")
                test_results.append(("场景 5", False, "未找到视频元素"))
        except Exception as e:
            print(f"  ❌ 场景 {i} 测试失败: {{e}}")
            test_results.append(("场景 {i}", False, str(e)))
        
        # 测试场景 6: 附加合法 MediaStream 时，成员端 <video> 能正常播放画面和（可选）音频
        print(f"\n测试场景 6: 附加合法 MediaStream 时，成员端 <video> 能正常播放画面和（可选）音频")
        try:
            # 检查视频元素（通常在点击开始共享后）
            # 如果场景提到"选择内容后"，说明需要先点击开始共享按钮
        ...
        # 测试场景 7: detachStream 后，视频区域清空或显示「等待流」状态
        print(f"\n测试场景 7: detachStream 后，视频区域清空或显示「等待流」状态")
        try:
            # 检查视频元素（通常在点击开始共享后）
            # 如果场景提到"选择内容后"，说明需要先点击开始共享按钮
        ...
        # 测试场景 8: 组件不依赖 WebRTC 细节，只关心 MediaStream 对象
        print(f"\n测试场景 8: 组件不依赖 WebRTC 细节，只关心 MediaStream 对象")
        try:
            # 场景: 组件不依赖 WebRTC 细节，只关心 MediaStream 对象
            # TODO: 根据具体场景实现测试逻辑
            print(f"  ⚠️  场景 8 需要根据具体需求实现")
            test_results.append(("场景 8", None, "需要手动实现"))
        except Exception as e:
            print(f"  ❌ 场景 {i} 测试失败: {{e}}")
            test_results.append(("场景 {i}", False, str(e)))
