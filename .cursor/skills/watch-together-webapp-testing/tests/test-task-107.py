...
def test_task_107():
    ...
    5. 房主与成员端都能在 onicecandidate 中发送 WEBRTC_ICE_CANDIDATE
    6. 接收方能正确解析并调用 addIceCandidate
    7. ICE 候选能通过 WebSocket 正确路由到目标连接
    8. 双方 connectionState 能变为 connected
    """
    # 测试结果截图保存到单独文件夹，便于归档与文档关联
    artifact_dir = 'backlog/test-results/task-107'
    os.makedirs(artifact_dir, exist_ok=True)
    scenarios = ['房主与成员端都能在 onicecandidate 中发送 WEBRTC_ICE_CANDIDATE', '接收方能正确解析并调用 addIceCandidate', 'ICE 候选能通过 WebSocket 正确路由到目标连接', '双方 connectionState 能变为 connected', '房主与成员端都能在 onicecandidate 中发送 WEBRTC_ICE_CANDIDATE', '接收方能正确解析并调用 addIceCandidate', 'ICE 候选能通过 WebSocket 正确路由到目标连接', '双方 connectionState 能变为 connected']
    print("=" * 60)
    print(f"测试 TASK-107")
    print("=" * 60)
    print()
    ...
    with sync_playwright() as p:
        ...
        # 测试场景 1: 房主与成员端都能在 onicecandidate 中发送 WEBRTC_ICE_CANDIDATE
        print(f"\n测试场景 1: 房主与成员端都能在 onicecandidate 中发送 WEBRTC_ICE_CANDIDATE")
        try:
            # 场景: 房主与成员端都能在 onicecandidate 中发送 WEBRTC_ICE_CANDIDATE
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 2: 接收方能正确解析并调用 addIceCandidate
        print(f"\n测试场景 2: 接收方能正确解析并调用 addIceCandidate")
        try:
            # 场景: 接收方能正确解析并调用 addIceCandidate
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 3: ICE 候选能通过 WebSocket 正确路由到目标连接
        print(f"\n测试场景 3: ICE 候选能通过 WebSocket 正确路由到目标连接")
        try:
            # 场景: ICE 候选能通过 WebSocket 正确路由到目标连接
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 4: 双方 connectionState 能变为 connected
        print(f"\n测试场景 4: 双方 connectionState 能变为 connected")
        try:
            # 场景: 双方 connectionState 能变为 connected
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 5: 房主与成员端都能在 onicecandidate 中发送 WEBRTC_ICE_CANDIDATE
        print(f"\n测试场景 5: 房主与成员端都能在 onicecandidate 中发送 WEBRTC_ICE_CANDIDATE")
        try:
            # 场景: 房主与成员端都能在 onicecandidate 中发送 WEBRTC_ICE_CANDIDATE
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 6: 接收方能正确解析并调用 addIceCandidate
        print(f"\n测试场景 6: 接收方能正确解析并调用 addIceCandidate")
        try:
            # 场景: 接收方能正确解析并调用 addIceCandidate
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 7: ICE 候选能通过 WebSocket 正确路由到目标连接
        print(f"\n测试场景 7: ICE 候选能通过 WebSocket 正确路由到目标连接")
        try:
            # 场景: ICE 候选能通过 WebSocket 正确路由到目标连接
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 8: 双方 connectionState 能变为 connected
        print(f"\n测试场景 8: 双方 connectionState 能变为 connected")
        try:
            # 场景: 双方 connectionState 能变为 connected
            # TODO: 根据具体场景实现测试逻辑
