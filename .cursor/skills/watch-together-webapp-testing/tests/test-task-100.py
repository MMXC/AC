...
def test_task_100():
    ...
    4. 多次开始/停止共享不会造成异常
    5. 点击「开始共享」后浏览器弹出屏幕/标签页选择对话框，用户可成功选择内容
    6. 选择内容后，本地预览 <video> 可实时显示采集画面
    7. 点击「停止共享」后，预览停止且 MediaStream 轨道已关闭
    8. 多次开始/停止共享不会造成异常
    """
    # 测试结果截图保存到单独文件夹，便于归档与文档关联
    artifact_dir = 'backlog/test-results/task-100'
    os.makedirs(artifact_dir, exist_ok=True)
    scenarios = ['点击「开始共享」后浏览器弹出屏幕/标签页选择对话框，用户可成功选择内容', '选择内容后，本地预览 <video> 可实时显示采集画面', '点击「停止共享」后，预览停止且 MediaStream 轨道已关闭', '多次开始/停止共享不会造成异常', '点击「开始共享」后浏览器弹出屏幕/标签页选择对话框，用户可成功选择内容', '选择内容后，本地预览 <video> 可实时显示采集画面', '点击「停止共享」后，预览停止且 MediaStream 轨道已关闭', '多次开始/停止共享不会造成异常']
    print("=" * 60)
    print(f"测试 TASK-100")
    print("=" * 60)
    print()
    ...
    with sync_playwright() as p:
        ...
        # 测试场景 1: 点击「开始共享」后浏览器弹出屏幕/标签页选择对话框，用户可成功选择内容
        print(f"\n测试场景 1: 点击「开始共享」后浏览器弹出屏幕/标签页选择对话框，用户可成功选择内容")
        try:
        ...
        # 测试场景 2: 选择内容后，本地预览 <video> 可实时显示采集画面
        print(f"\n测试场景 2: 选择内容后，本地预览 <video> 可实时显示采集画面")
        try:
            # 检查视频元素（通常在点击开始共享后）
            # 如果场景提到"选择内容后"，说明需要先点击开始共享按钮
            if '选择内容后' in '选择内容后，本地预览 <video> 可实时显示采集画面' or '选择' in '选择内容后，本地预览 <video> 可实时显示采集画面':
                # 先查找并点击"开始共享"按钮
                start_button = page.locator('button:has-text("开始共享"), button:has-text("开始")')
                if start_button.count() > 0 and start_button.first.is_visible():
                    print(f"  ✅ 找到开始共享按钮，点击...")
        ...
        # 测试场景 3: 点击「停止共享」后，预览停止且 MediaStream 轨道已关闭
        print(f"\n测试场景 3: 点击「停止共享」后，预览停止且 MediaStream 轨道已关闭")
        try:
            # 检查视频元素（通常在点击开始共享后）
            # 如果场景提到"选择内容后"，说明需要先点击开始共享按钮
            if '选择内容后' in '点击「停止共享」后，预览停止且 MediaStream 轨道已关闭' or '选择' in '点击「停止共享」后，预览停止且 MediaStream 轨道已关闭':
                # 先查找并点击"开始共享"按钮
                start_button = page.locator('button:has-text("开始共享"), button:has-text("开始")')
                if start_button.count() > 0 and start_button.first.is_visible():
                    print(f"  ✅ 找到开始共享按钮，点击...")
        ...
        try:
            ...
            if stop_button.count() > 0:
                ...
                if streams_closed:
                    ...
                    test_results.append(("场景 4", True, "MediaStream 已关闭"))
                else:
                    print(f"  ⚠️  MediaStream 可能未完全关闭")
                    test_results.append(("场景 4", None, "MediaStream 可能未完全关闭"))
            else:
                print(f"  ⚠️  未找到停止/关闭按钮")
                test_results.append(("场景 4", None, "未找到停止按钮"))
        except Exception as e:
            print(f"  ❌ 场景 {i} 测试失败: {{e}}")
            test_results.append(("场景 {i}", False, str(e)))
        
        # 测试场景 5: 点击「开始共享」后浏览器弹出屏幕/标签页选择对话框，用户可成功选择内容
        print(f"\n测试场景 5: 点击「开始共享」后浏览器弹出屏幕/标签页选择对话框，用户可成功选择内容")
        try:
            # 检查对话框/弹出窗口（如屏幕共享选择对话框）
            # 先查找并点击"开始共享"按钮
        ...
        # 测试场景 6: 选择内容后，本地预览 <video> 可实时显示采集画面
        print(f"\n测试场景 6: 选择内容后，本地预览 <video> 可实时显示采集画面")
        try:
            # 检查视频元素（通常在点击开始共享后）
            # 如果场景提到"选择内容后"，说明需要先点击开始共享按钮
        ...
        # 测试场景 7: 点击「停止共享」后，预览停止且 MediaStream 轨道已关闭
        print(f"\n测试场景 7: 点击「停止共享」后，预览停止且 MediaStream 轨道已关闭")
        try:
            # 检查视频元素（通常在点击开始共享后）
            # 如果场景提到"选择内容后"，说明需要先点击开始共享按钮
        ...
        try:
            ...
            if stop_button.count() > 0:
                ...
                if streams_closed:
                    print(f"  ✅ MediaStream 已正确关闭")
                    test_results.append(("场景 8", True, "MediaStream 已关闭"))
                else:
                    print(f"  ⚠️  MediaStream 可能未完全关闭")
                    test_results.append(("场景 8", None, "MediaStream 可能未完全关闭"))
            else:
                print(f"  ⚠️  未找到停止/关闭按钮")
                test_results.append(("场景 8", None, "未找到停止按钮"))
        except Exception as e:
            print(f"  ❌ 场景 {i} 测试失败: {{e}}")
            test_results.append(("场景 {i}", False, str(e)))
