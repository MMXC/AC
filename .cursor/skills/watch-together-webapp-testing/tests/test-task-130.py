...
def test_task_130():
    ...
    4. 房主端建立/重试 WebRTC 连接时，仅向 getMembersList() 中且非当前用户的 member.id 发送 Offer
    5. 新加入成员在房主端列表更新后能收到房主发出的 WebRTC 信令（Offer），成员端能建立画面流
    6. 用户离开后，房主端不再向该 userId 发送信令（服务端「信令目标用户不在线」日志不再出现或仅偶发）
    """
    # 测试结果截图保存到单独文件夹，便于归档与文档关联
    artifact_dir = 'backlog/test-results/task-130'
    os.makedirs(artifact_dir, exist_ok=True)
    scenarios = ['房主端建立/重试 WebRTC 连接时，仅向 getMembersList() 中且非当前用户的 member.id 发送 Offer', '新加入成员在房主端列表更新后能收到房主发出的 WebRTC 信令（Offer），成员端能建立画面流', '用户离开后，房主端不再向该 userId 发送信令（服务端「信令目标用户不在线」日志不再出现或仅偶发）', '房主端建立/重试 WebRTC 连接时，仅向 getMembersList() 中且非当前用户的 member.id 发送 Offer', '新加入成员在房主端列表更新后能收到房主发出的 WebRTC 信令（Offer），成员端能建立画面流', '用户离开后，房主端不再向该 userId 发送信令（服务端「信令目标用户不在线」日志不再出现或仅偶发）']
    print("=" * 60)
    print(f"测试 TASK-130")
    print("=" * 60)
    print()
    ...
    with sync_playwright() as p:
        ...
        # 测试场景 1: 房主端建立/重试 WebRTC 连接时，仅向 getMembersList() 中且非当前用户的 member.id 发送 Offer
        print(f"\n测试场景 1: 房主端建立/重试 WebRTC 连接时，仅向 getMembersList() 中且非当前用户的 member.id 发送 Offer")
        try:
            # 断言房主端成员列表包含当前成员（与 TASK-130 验收一致：仅向当前成员发 Offer）
            member_list_selector = '#membersList, #memberList, .member-list, [data-testid="member-list"], ul.members'
            found_on_host = False
            host_list_text = ""
            ...
            if found_on_host:
                test_results.append(("场景 1", True, "房主端成员列表包含当前成员"))
            else:
                test_results.append(("场景 1", False, "房主端成员列表未含预期成员"))
        except Exception as e:
            print(f"  ❌ 场景 1 测试失败: {e}")
            test_results.append(("场景 1", False, str(e)))
        
        # 测试场景 2: 新加入成员在房主端列表更新后能收到房主发出的 WebRTC 信令（Offer），成员端能建立画面流
        print(f"\n测试场景 2: 新加入成员在房主端列表更新后能收到房主发出的 WebRTC 信令（Offer），成员端能建立画面流")
        try:
            # 检查房主端/成员端成员列表是否包含预期成员
            member_list_selector = '#membersList, #memberList, .member-list, [data-testid="member-list"], ul.members'
            found_on_host = False
            ...
            if found_on_host or found_on_member:
                test_results.append(("场景 2", True, "成员列表包含预期成员"))
            else:
        ...
        # 测试场景 3: 用户离开后，房主端不再向该 userId 发送信令（服务端「信令目标用户不在线」日志不再出现或仅偶发）
        print(f"\n测试场景 3: 用户离开后，房主端不再向该 userId 发送信令（服务端「信令目标用户不在线」日志不再出现或仅偶发）")
        try:
            # 场景: 用户离开后，房主端不再向该 userId 发送信令（服务端「信令目标用户不在线」日志不再出现或仅偶发）
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 4: 房主端建立/重试 WebRTC 连接时，仅向 getMembersList() 中且非当前用户的 member.id 发送 Offer
        print(f"\n测试场景 4: 房主端建立/重试 WebRTC 连接时，仅向 getMembersList() 中且非当前用户的 member.id 发送 Offer")
        try:
            # 断言房主端成员列表包含当前成员（与 TASK-130 验收一致）
            member_list_selector = '#membersList, #memberList, .member-list, [data-testid="member-list"], ul.members'
            found_on_host = False
            host_list_text = ""
            if page.locator(member_list_selector).count() > 0:
                ...
                print(f"  {'✅' if found_on_host else '⚠️'} 房主端成员列表: {host_list_text[:100]}...")
            if found_on_host:
                test_results.append(("场景 4", True, "房主端成员列表包含当前成员"))
            else:
                test_results.append(("场景 4", False, "房主端成员列表未含预期成员"))
        except Exception as e:
            print(f"  ❌ 场景 4 测试失败: {e}")
            test_results.append(("场景 4", False, str(e)))
        
        # 测试场景 5: 新加入成员在房主端列表更新后能收到房主发出的 WebRTC 信令（Offer），成员端能建立画面流
        print(f"\n测试场景 5: 新加入成员在房主端列表更新后能收到房主发出的 WebRTC 信令（Offer），成员端能建立画面流")
        try:
            # 检查房主端/成员端成员列表是否包含预期成员
            member_list_selector = '#membersList, #memberList, .member-list, [data-testid="member-list"], ul.members'
            ...
            if page_member and page_member.locator(member_list_selector).count() > 0:
                ...
                print(f"  {'✅' if found_on_member else '⚠️'} 成员端成员列表: {member_list_text[:100]}...")
            if found_on_host or found_on_member:
                test_results.append(("场景 5", True, "成员列表包含预期成员"))
            else:
                test_results.append(("场景 5", None, "成员列表需根据页面选择器调整"))
        except Exception as e:
            print(f"  ❌ 场景 5 测试失败: {e}")
            test_results.append(("场景 5", False, str(e)))
        
        # 测试场景 6: 用户离开后，房主端不再向该 userId 发送信令（服务端「信令目标用户不在线」日志不再出现或仅偶发）
        print(f"\n测试场景 6: 用户离开后，房主端不再向该 userId 发送信令（服务端「信令目标用户不在线」日志不再出现或仅偶发）")
        try:
            # 场景: 用户离开后，房主端不再向该 userId 发送信令（服务端「信令目标用户不在线」日志不再出现或仅偶发）
            # TODO: 根据具体场景实现测试逻辑
            ...
            print(f"  ❌ 场景 6 测试失败: {e}")
            test_results.append(("场景 6", False, str(e)))
