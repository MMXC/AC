...
def test_task_106():
    ...
    artifact_dir = 'backlog/test-results/task-106'
    os.makedirs(artifact_dir, exist_ok=True)
    scenarios = ['能正确解析并处理 WebSocket 收到的 WEBRTC_OFFER', '能创建 RTCPeerConnection 并 setRemoteDescription(offer)', 'createAnswer 成功后能通过 WebSocket 发送 WEBRTC_ANSWER', 'WEBRTC_ANSWER 的 toUserId 指向房主']
    print("=" * 60)
    print(f"测试 TASK-106")
    print("=" * 60)
    print()
    ...
    with sync_playwright() as p:
        ...
        # 测试场景 1: 能正确解析并处理 WebSocket 收到的 WEBRTC_OFFER
        print(f"\n测试场景 1: 能正确解析并处理 WebSocket 收到的 WEBRTC_OFFER")
        try:
            # 场景: 能正确解析并处理 WebSocket 收到的 WEBRTC_OFFER
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 2: 能创建 RTCPeerConnection 并 setRemoteDescription(offer)
        print(f"\n测试场景 2: 能创建 RTCPeerConnection 并 setRemoteDescription(offer)")
        try:
            # 场景: 能创建 RTCPeerConnection 并 setRemoteDescription(offer)
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 3: createAnswer 成功后能通过 WebSocket 发送 WEBRTC_ANSWER
        print(f"\n测试场景 3: createAnswer 成功后能通过 WebSocket 发送 WEBRTC_ANSWER")
        try:
            # 场景: createAnswer 成功后能通过 WebSocket 发送 WEBRTC_ANSWER
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 4: WEBRTC_ANSWER 的 toUserId 指向房主
        print(f"\n测试场景 4: WEBRTC_ANSWER 的 toUserId 指向房主")
        try:
            # 场景: WEBRTC_ANSWER 的 toUserId 指向房主
            # TODO: 根据具体场景实现测试逻辑
