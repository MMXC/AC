...
def test_task_110():
    ...
    5. 房主端为每个成员创建并跟踪独立的 PeerConnection（以 userId 为 key）
    6. 信令层能为每个成员正确路由 WebRTC 信令
    7. 新成员加入时可增量建立连接，不影响已有连接
    8. 成员离开时，房主端能关闭对应 PeerConnection 并释放资源
    """
    # 测试结果截图保存到单独文件夹，便于归档与文档关联
    artifact_dir = 'backlog/test-results/task-110'
    os.makedirs(artifact_dir, exist_ok=True)
    scenarios = ['房主端为每个成员创建并跟踪独立的 PeerConnection（以 userId 为 key）', '信令层能为每个成员正确路由 WebRTC 信令', '新成员加入时可增量建立连接，不影响已有连接', '成员离开时，房主端能关闭对应 PeerConnection 并释放资源', '房主端为每个成员创建并跟踪独立的 PeerConnection（以 userId 为 key）', '信令层能为每个成员正确路由 WebRTC 信令', '新成员加入时可增量建立连接，不影响已有连接', '成员离开时，房主端能关闭对应 PeerConnection 并释放资源']
    print("=" * 60)
    print(f"测试 TASK-110")
    print("=" * 60)
    print()
    ...
    with sync_playwright() as p:
        ...
        # 测试场景 1: 房主端为每个成员创建并跟踪独立的 PeerConnection（以 userId 为 key）
        print(f"\n测试场景 1: 房主端为每个成员创建并跟踪独立的 PeerConnection（以 userId 为 key）")
        try:
            # 场景: 房主端为每个成员创建并跟踪独立的 PeerConnection（以 userId 为 key）
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 2: 信令层能为每个成员正确路由 WebRTC 信令
        print(f"\n测试场景 2: 信令层能为每个成员正确路由 WebRTC 信令")
        try:
            # 场景: 信令层能为每个成员正确路由 WebRTC 信令
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 3: 新成员加入时可增量建立连接，不影响已有连接
        print(f"\n测试场景 3: 新成员加入时可增量建立连接，不影响已有连接")
        try:
            # 场景: 新成员加入时可增量建立连接，不影响已有连接
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 4: 成员离开时，房主端能关闭对应 PeerConnection 并释放资源
        print(f"\n测试场景 4: 成员离开时，房主端能关闭对应 PeerConnection 并释放资源")
        try:
            # 检查停止/关闭功能
            # 查找停止按钮
        ...
        # 测试场景 5: 房主端为每个成员创建并跟踪独立的 PeerConnection（以 userId 为 key）
        print(f"\n测试场景 5: 房主端为每个成员创建并跟踪独立的 PeerConnection（以 userId 为 key）")
        try:
            # 场景: 房主端为每个成员创建并跟踪独立的 PeerConnection（以 userId 为 key）
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 6: 信令层能为每个成员正确路由 WebRTC 信令
        print(f"\n测试场景 6: 信令层能为每个成员正确路由 WebRTC 信令")
        try:
            # 场景: 信令层能为每个成员正确路由 WebRTC 信令
            # TODO: 根据具体场景实现测试逻辑
        ...
        # 测试场景 8: 成员离开时，房主端能关闭对应 PeerConnection 并释放资源
        print(f"\n测试场景 8: 成员离开时，房主端能关闭对应 PeerConnection 并释放资源")
        try:
            # 检查停止/关闭功能
            # 查找停止按钮
