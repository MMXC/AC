...
export async function ipRateLimit(
  ...
  const result = await checkRateLimit(
    key,
    RATE_LIMIT_CONFIG.ipLimit.maxRequests,
    RATE_LIMIT_CONFIG.ipLimit.windowMs
  );
...
export async function userRateLimit(
  ...
  const result = await checkRateLimit(
    key,
    RATE_LIMIT_CONFIG.userLimit.maxRequests,
    RATE_LIMIT_CONFIG.userLimit.windowMs
  );
...
export async function rateLimit(
  ...
  // 先检查 IP 限流
  await ipRateLimit(req, res, (err?: any) => {
    if (err) {
      return next(err);
    }
    // IP 限流通过后，检查用户限流
