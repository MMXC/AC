  ...
  try {
    ...
    ws.on('message', async (data: Buffer) => {
      ...
      try {
        ...
        // 处理 WebRTC 信令消息（权限控制）
        if (typeof message.type === 'string' && message.type.startsWith('WEBRTC_')) {
          await handleWebRTCSignalingMessage(ws, message as WebRTCSignalingMessageBase, roomId, userId);
          return;
        }
